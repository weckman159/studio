...
import { Sparkles, Users, Rss, Store, Car, Zap, Search, GaugeCircle, BatteryCharging, Palette, Info, Newspaper, MoreHorizontal, Heart, MessageCircle, Send, Shield, BarChart, ChevronRight, Gem, Infinity, Filter, Bell, Menu } from 'lucide-react';
        ...
        <aside className="w-[260px] hidden md:flex flex-none bg-background-dark text-white flex-col justify-between z-40 overflow-y-auto px-6 py-8 border-r border-primary/20 holographic-panel-dark">
            <div className="flex flex-col">
                ...
                <nav className="flex flex-col gap-3">
                    {navItems.map(item => (
                        <Link key={item.label} href={item.href} className={`group flex items-center gap-4 px-5 py-3 rounded-lg transition-all duration-300 border-l-4 ${pathname.startsWith(item.href) && item.href !== '/' || pathname === item.href ? 'bg-primary/10 border-primary text-primary glow' : 'text-white/70 hover:text-white hover:bg-primary/10 hover:border-primary/50 border-transparent'}`}>
                            {item.icon}
         ...
         <header className="h-16 flex-none bg-background-dark/80 backdrop-blur-md border-b border-primary/20 flex items-center justify-between px-8 z-30">
            ...
            <div className="flex items-center gap-5">
                <div className="relative w-64">
                    ...
                    <input className="w-full bg-white/5 border border-primary/30 rounded-full py-2 pl-10 pr-4 text-sm focus:ring-2 focus:ring-primary/50 placeholder-primary/50 text-white" placeholder="Поиск..." type="text" />
                </div>
                {user ? (
                <>
                    <button className="relative group text-white/70 hover:text-primary transition-colors flex items-center justify-center">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                             <div className="w-10 h-10 rounded-full bg-cover bg-center border border-primary/50 shadow-[0_0_10px_rgba(10,170,255,0.3)] cursor-pointer hover:scale-105 transition-transform" style={{backgroundImage: `url(${user.photoURL || 'https://i.pravatar.cc/40'})`}}></div>
                        </DropdownMenuTrigger>
