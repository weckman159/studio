...
import {
    ...
    MoreHorizontal, Heart, MessageCircle, Send, Shield, BarChart, ChevronRight, Gem, Infinity, Star,
} from 'lucide-react';
    ...
    return (
        <section className="w-full relative rounded-2xl overflow-hidden holographic-panel group min-h-[500px] flex flex-col justify-end">
            ...
            <div className="absolute inset-0 perspective-[1000px] flex items-center justify-center overflow-hidden z-10 pointer-events-none">
                <div ref={carModelRef} className="w-[90%] aspect-[16/9] transition-transform duration-700 group-hover:scale-105" style={{ backgroundImage: "url('https://picsum.photos/seed/gtsilver/1200/675')", backgroundSize: 'contain', backgroundRepeat: 'no-repeat', backgroundPosition: 'center', transform: 'rotateY(0deg) scale(1.1)', transformStyle: 'preserve-3d' }}>
                     <div className="absolute top-0 left-0 p-4">
                        <Star className="text-yellow-400 text-5xl drop-shadow-[0_0_15px_rgba(250,204,21,0.6)]" fill="currentColor"/>
                    </div>
                    <div className="absolute bottom-[20%] left-[5%] text-white text-shadow-md p-3 rounded-lg holographic-panel pointer-events-auto cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-z-10">
                        <GaugeCircle className="text-primary text-[28px] mb-1" />
                        <p className="text-xl font-bold text-primary">992 HP</p>
                        <p className="text-xs text-white/70">Performance Mode</p>
                    </div>
                    <div className="absolute top-[10%] right-[5%] text-white text-shadow-md p-3 rounded-lg holographic-panel pointer-events-auto cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-z-10">
                        <BatteryCharging className="text-primary text-[28px] mb-1" />
                        <p className="text-xl font-bold text-primary">Hybrid</p>
                        <p className="text-xs text-white/70">Drivetrain</p>
                    </div>
                </div>
            </div>
            ...
            <article className="p-5 rounded-xl bg-primary/5 border border-primary/20 hover:border-primary/50 transition-colors holographic-panel-dark">
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div><h4 className="font-bold text-sm text-white">Julia Stevens</h4><div className="flex items-center gap-2 text-xs text-white/70"><span>San Francisco, CA</span><span>•</span><span>2 hours ago</span></div></div>
                    </div>
            ...
            <article className="p-5 rounded-xl bg-primary/5 border border-primary/20 hover:border-primary/50 transition-colors holographic-panel-dark">
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div><h4 className="font-bold text-sm text-white">AutoSphere Official</h4><div className="flex items-center gap-2 text-xs text-white/70"><span className="bg-primary/20 text-primary px-1 rounded text-[10px] font-bold">ADMIN</span><span>•</span><span>5 hours ago</span></div></div>
                    </div>
                    <button className="text-white/70 hover:text-primary"><MoreHorizontal /></button>
                </div>
