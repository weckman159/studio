...
const LeftSidebar = () => {
    const navItems = [
        { label: 'Главная', icon: <Sparkles className="group-hover:scale-110 transition-transform text-[20px] filled" />, active: true },
        { label: 'Сообщества', icon: <Users className="group-hover:scale-110 transition-transform text-[20px]" /> },
        { label: 'Блоги', icon: <Rss className="group-hover:scale-110 transition-transform text-[20px]" /> },
        { label: 'Маркетплейс', icon: <Store className="group-hover:scale-110 transition-transform text-[20px]" /> },
        { label: 'Автомобили', icon: <Car className="group-hover:scale-110 transition-transform text-[20px]" /> },
    ];

    return (
        <aside className="w-[260px] flex-none bg-background-dark text-white flex flex-col justify-between z-40 sidebar-scroll overflow-y-auto px-6 py-8 border-r border-primary/20 holographic-panel-dark">
            <div className="flex flex-col">
                ...
                <nav className="flex flex-col gap-3">
                    {navItems.map(item => (
                        <Link key={item.label} href="#" className={`group flex items-center gap-4 px-5 py-3 rounded-lg transition-all duration-300 border-l-4 ${item.active ? 'bg-primary/10 border-primary text-primary glow' : 'text-white/70 hover:text-white hover:bg-primary/10 hover:border-primary/50 border-transparent'}`}>
                            {item.icon}
...
const RightSidebar = () => {
    return (
        <div className="w-[320px] flex-none border-l border-primary/20 bg-background-dark/20 p-6 overflow-y-auto scroll-effect flex flex-col gap-6 z-20 backdrop-blur-sm">
            ...
            <div className="flex-none flex flex-col">
                ...
                <div className="flex flex-col gap-2">
                    {[
                        ...
                        { name: 'Lucid Air Sapphire', spec: '1,234 HP • Sedan' },
                        { name: 'Porsche Taycan T-GT', spec: '1,092 HP • Track' },
                    ].map((car, index) => (
            ...
            <div className="flex-none">
                ...
                <div className="flex flex-col gap-4">
                    {[
                        ...
                        { cat: 'INDUSTRY', time: '1d ago', title: 'Global charging standard adoption accelerates in EU.' }
                    ].map((news, index) => (
                        <React.Fragment key={index}>
                            ...
                            {index < 2 && <div className="h-px bg-white/5 w-full"></div>}
                        </React.Fragment>
                    ))}
                </div>
            </div>
            ...
            <main className="flex-1 flex flex-col min-w-0 bg-transparent relative overflow-hidden">
                <header className="h-16 flex-none bg-background-dark/80 backdrop-blur-md border-b border-primary/20 flex items-center justify-between px-8 z-30">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="relative w-64">
                            ...
                            <input className="w-full bg-white/5 border border-primary/30 rounded-full py-2 pl-10 pr-4 text-sm focus:ring-2 focus:ring-primary/50 placeholder-primary/50 text-white" placeholder="Search vehicle diagnostics..." type="text"/>
                        </div>
                ...
                <footer className="flex-none py-4 border-t border-primary/20 bg-background-dark/80 backdrop-blur-md z-30">
                    <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 px-8">
                        ...
                        <div className="flex items-center gap-6">
                            <a className="text-xs font-medium text-white/70 hover:text-primary transition-colors" href="#">About</a>
                            <a className="text-xs font-medium text-white/70 hover:text-primary transition-colors" href="#">Support Matrix</a>
                            <a className="text-xs font-medium text-white/70 hover:text-primary transition-colors" href="#">Data Security Protocol</a>
                            <a className="text-xs font-medium text-white/70 hover:text-primary transition-colors" href="#">Terms of Service</a>
                        </div>
