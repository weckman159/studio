...
export default function AdminUsersPage() {
    ...
    const performAction = async (targetUserId: string, action: 'set-role' | 'ban' | 'unban', payload?: any) => {
        if (!adminUser) return;
                                    ...
                                    <TableCell className="text-right">
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                ...
                                                <DropdownMenuItem onClick={() => openConfirmation(`Сделать ${user.displayName} админом?`, 'Это даст ему полные права.', () => performAction(user.id, 'set-role', { role: 'admin' }))} disabled={user.roles?.isAdmin}>
                                                    <Shield className="mr-2 h-4 w-4" />Сделать админом
                                                </DropdownMenuItem>
                                                <DropdownMenuItem onClick={() => openConfirmation(`Снять права админа с ${user.displayName}?`, 'Пользователь станет обычным юзером.', () => performAction(user.id, 'set-role', { role: 'user' }))} disabled={!user.roles?.isAdmin}>
                 ...
                 <AlertDialog open={actionDialog.isOpen} onOpenChange={(isOpen) => !isOpen && setActionDialog(null)}>
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>{actionDialog.description}</AlertDialogHeader>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
